...
//-------listar cuentas para paginas
	public function ListAccounttw2(){
		$Accounts = Accounttw::all();
		$accounts =""; 
		if (count($accounts) ==0)
		{
		...
		else
		{
			...
			$encontrados = '<select id="account2" name="modal_menu" multiple="multiple" class="populate placeholder">
				        		'.$accounts.'					
								</select>';
		}
...
public function Trending2()
	{ 
		...
		if (Request::ajax()) 
		{
				...
				$encontrados = '<td id="ClickWordList2" class="fc-header-center">
				        		'.$trnd.'					
								</td>';
